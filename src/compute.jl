...
"""
function create_model_N3(N_spin_orbital,symmetry,n_target,interaction,chemical_potential,e_fns;particle_hole_symmetric=false,w_mode="fluc",cal_Eeff=cal_Eeff_test,N_Ueff=0)
    ...
    model=Model(3,N_spin_orbital,symmetry,n_target,w_para,G12_para,β_para,interaction,chemical_potential,e_fns,options,obs)
    # set options
    # set density mode
    if(length(n_target)>0)
        # model.options
...
set_para(model,para)
"""
function set_para(mode::Model,para)
...
function compute_local(model::Model)
    ...
    @set_obs model G12ασ βασ  w g12ασ Slocασ  Δασ pmatwασ g12matwSασ g11matwSασ
...
function compute_momentum(model::Model)
    @get_obs model nασ Δασ βασ eασ G12ασ g12ασ Slocασ pmatwασ w
    ...
    # model.obs
    @set_obs model  Etotal Ek Eloc Echem Aασ_below Aασ_above αασ nk nn g33matwασ Ekασ Zασ μασ
    Etotal
end
